(function () {
  ...
  var medrent = colorbrewer.YlOrRd[7]; //median rent
  ...
  L.Util.ajax('geodata/allcensusacsdata.json').then( function(data) {
    ...
    for (key in census2010) {
        var obj = census2010[key].medianrent;
        rentarray2010.push(obj);
    }

    for (key in census2000) {
      var obj = census2000[key].medianrent;
      rentarray2000.push(obj);
    }

    for (key in census1990) {
        var obj = census1990[key].medianrent;
        rentarray1990.push(obj);
    }

    // console.log(rentarray);
    var incstats2010 = new geostats(rentarray2010);
    var a2010 = incstats2010.getClassQuantile(5);
    var range2010 = incstats2010.ranges;

    var incstats2000 = new geostats(rentarray2000);
    var a2000 = incstats2000.getClassQuantile(5);
    var range2000 = incstats2000.ranges;

    var incstats1990 = new geostats(rentarray1990);
    var a1990 = incstats1990.getClassQuantile(5);
    var range1990 = incstats1990.ranges;
    ...
    var tracts2010 = L.geoJson.ajax("geodata/tracts2010.json",{
      ...
      function(feature){
        ...
        try {
          var d = data[2008][featureId].medianrent;
          // console.log(d);
        } catch (e) {
          // console.log(e)
        }
        var fill = d > a2010[5] ? medrent[5] :
                   d > a2010[4] ? medrent[4] :
                   d > a2010[3] ? medrent[3] :
                   d > a2010[2] ? medrent[2] :
                   d > a2010[1] ? medrent[1] :
    ...
    var tracts2000 = L.geoJson.ajax("geodata/tracts2000.json",{
      ...
      function(feature){
        var featureId = feature.id;
        try {
          var d = data[2000][featureId].medianrent;
          // console.log(d);
        } catch (e) {
          // console.log(e)
        }
        var fill = d > a2000[5] ? medrent[5] :
                   d > a2000[4] ? medrent[4] :
                   d > a2000[3] ? medrent[3] :
                   d > a2000[2] ? medrent[2] :
                   d > a2000[1] ? medrent[1] :
    ...
    var tracts1990 = L.geoJson.ajax("geodata/tracts1990.json",{
      ...
      function(feature){
        var featureId = feature.id;
        try {
          var d = data[1990][featureId].medianrent;
          // console.log(d);
        } catch (e) {
          // console.log(e)
        }
        var fill = d > a1990[5] ? medrent[5] :
                   d > a1990[4] ? medrent[4] :
                   d > a1990[3] ? medrent[3] :
                   d > a1990[2] ? medrent[2] :
                   d > a1990[1] ? medrent[1] :
  ...
  // add legends
    ...
    legend2010.onAdd = function (map) {
      ...
      div.innerHTML += '<h4>Median rent by tract 2010</h4>';
      for (var i = 0; i < range2010.length; i++) {
          div.innerHTML +=
              '<i style="background:' + medrent[i] + ';"></i> ' + range2010[i] + '<br>';
      }
      return div;
    };   
    ...
    legend2000.onAdd = function (map) {
      var div = L.DomUtil.create('div', 'info legend');
      div.innerHTML += '<h4>Median rent by tract 2000</h4>';
      for (var i = 0; i < range2000.length; i++) {
          div.innerHTML +=
              '<i style="background:' + medrent[i] + ';"></i> ' + range2000[i] + '<br>';
      }
      return div;
    };   
    ...
    legend1990.onAdd = function (map) {
      var div = L.DomUtil.create('div', 'info legend');
      div.innerHTML += '<h4>Median rent by tract 1990</h4>';
      for (var i = 0; i < range1990.length; i++) {
          div.innerHTML +=
              '<i style="background:' + medrent[i] + ';"></i> ' + range1990[i] + '<br>';
      }
      return div;
    };   
